...
# Shared httpx timeout and retry settings for external calls
...
async def _http_post_with_retry(
    ...
    *,
    ...
    """Make an HTTP POST request with retry logic for transient DNS/network errors."""
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            ...
            return response
        except (httpx.ConnectError, httpx.TimeoutException, httpx.PoolTimeout, OSError) as exc:
            last_exc = exc
            logger.warning(
                "[%s] Attempt %d/%d failed: %s — %s",
                tag, attempt, _MAX_RETRIES, type(exc).__name__, exc,
            )
...
async def callback(
    ...
    try:
        ...
        try:
            token_response = await _http_post_with_retry(
                token_url,
                data=token_data,
...
async def exchange_platform_token(
    ...
    try:
        verify_response = await _http_post_with_retry(
            verify_url,
        ...
        raise HTTPException(
            ...
            detail=f"Unable to verify platform token: {type(exc).__name__} — {exc}",
        ) from exc
