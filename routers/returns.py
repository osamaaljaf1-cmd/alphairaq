...
async def _delete_owner_filter(db: AsyncSession, current_user: UserResponse) -> Optional[str]:
    """Resolve the user_id to pass to ReturnsService.delete(): None lets a
    privileged caller delete any return, a string restricts to their own.

    Reps may only delete their own returns. Everyone else (admin, accounting,
    manager, delivery, or any account with no representative row at all,
    e.g. an app_user-only account) can delete any return — mirrors the
    same split used for orders in routers/orders.py."""
...
async def delete_returnss_batch(
    ...
    """Delete multiple returnss by their IDs. Requires can_delete permission
    on the returns page; reps are further restricted to their own returns
    (see _delete_owner_filter)."""
    await require_permission(db, current_user, "returns", "delete")
...
async def delete_returns(
    ...
    """Delete a single returns by ID. Requires can_delete permission on the
    returns page; reps are further restricted to their own returns (see
