...
async def get_jwks() -> Dict[str, Any]:
    """Get JWKS (JSON Web Key Set) from OIDC provider with retry logic."""
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            async with httpx.AsyncClient(timeout=_HTTP_TIMEOUT) as client:
                logger.info(f"Fetching JWKS from: {jwks_url} (attempt {attempt}/{_MAX_RETRIES})")
                response = await client.get(jwks_url)
                response.raise_for_status()
                jwks_data = response.json()
                logger.info(f"Successfully fetched JWKS with {len(jwks_data.get('keys', []))} keys")
                return jwks_data
        except (httpx.ConnectError, httpx.TimeoutException, httpx.PoolTimeout, OSError) as e:
            last_exc = e
            logger.warning(
                f"[get_jwks] Attempt {attempt}/{_MAX_RETRIES} failed (transient): {type(e).__name__}: {e}"
            )
    ...
    logger.error(f"All {_MAX_RETRIES} attempts to fetch JWKS failed. Last error: {last_exc}")
    raise Exception("Unable to retrieve authentication keys")
